...
let refuseLoanUseCase: RefuseLoanUseCase;
...
describe('Refuse Loan', () => {
  beforeEach(async () => {
    ...
    refuseLoanUseCase = new RefuseLoanUseCase(loansRepositoryInMemory);
  ...
  it('should be able to refuse a loan', async () => {
    const result = await refuseLoanUseCase.execute({
      id: loan_id,
      user_id: contact_id
    });
  ...
  it('should not be able to refuse a nonexistent loan', async () => {
    await expect(
      refuseLoanUseCase.execute({ id: '123', user_id: contact_id })
    ).rejects.toEqual(new AppError('Loan not found'));
  });

  it('should not allow user that created the loan to refuse it', async () => {
    await expect(
      refuseLoanUseCase.execute({ id: loan_id, user_id })
    ).rejects.toEqual(
      new AppError('This user is not allowed to refuse this loan', 401)
    );
  });
});
