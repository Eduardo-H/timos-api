...
describe('Refuse Payment Controller', () => {
  beforeAll(async () => {
  ...
  it('should be able to refuse a payement', async () => {
    const response = await request(app)
      .patch(`/loans/payment/${payment_id}/refuse`)
      .set({
        Authorization: `Bearer ${contact_token}`
      });
  ...
  it('should not be able to refuse a nonexistent payment', async () => {
    const response = await request(app)
      .patch('/loans/payment/868272c3-c308-44e8-9a53-e0ccf61e9639/refuse')
      .set({
        Authorization: `Bearer ${contact_token}`
      });
